...
const selectedEventSelector = (state) => {
  const { events, selectedEventId } = state.calendarAvailability;
...
function ApplicationsCalendar() {
  ...
  const { events, isDrawerOpen, selectedRange, option, selectable, hasPreviousEvents, weekNumber } = useSelector(
    (state) => state.calendarAvailability
  );
  ...
  const handleRangeSelect = async(arg) => {
    ...
    if(selectable){
      ...
      // Call api
      try {
        const body = {date: arg.start.toISOString(), partnerId: user.person.id};
  ...
  useEffect(() => {
    dispatch(getEvents(user.person.id));
  }, [dispatch]);
  ...
  const handleDatePrev = () => {
    ...
    if (calItem) {
      ...
      dispatch(getEvents(user.person.id, weekNumber-1));
    }
  };
