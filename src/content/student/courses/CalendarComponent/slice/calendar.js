...
const slice = createSlice({
  ...
  reducers: {
    ...
    AddDisponibility(state, action) {
      const { start, end } = action.payload;

      let event = {
        id: uuidv4(),
        allDay: false,
        color: color_availabilities, // Cambiar color
        description: '',
        end: end,
        start:start,
        title: '',
      ...
      console.log("Se agrego evento",event);
      state.events = [...state.events, event];
    },
...
export const AddDisponibility = (start, end) => async (dispatch) =>{
  console.log('AddDisponibility');
  console.log(start, end);
  try {
    dispatch(
      slice.actions.AddDisponibility({
        start,
        end
      })
    );
...
export const cleanEvents = () => (dispatch) => {
  console.log("cleanEvents");
  try {
    dispatch(slice.actions.cleanEvents());
  } catch (error) {
    console.error(error);
  }
}
...
export const getEventos = () => async(dispatch) => {
  ...
  try {
    dispatch(slice.actions.getEventos());
  } catch (e) {
    console.error(e);
  }
}
